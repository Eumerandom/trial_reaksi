    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')"
                class="text-gray-800 border-l-4 {{ request()->routeIs('dashboard') ? 'border-zinc-700!' : 'border-transparent' }}">
                {{ __('Beranda') }}
            </x-responsive-nav-link>

            <x-responsive-nav-link href="{{ route('product.index') }}" :active="request()->routeIs('product.index')"
                class="text-gray-800 border-l-4 {{ request()->routeIs('product.index') ? 'border-zinc-700!' : 'border-transparent' }}">
                {{ __('Produk') }}
            </x-responsive-nav-link>

            <x-responsive-nav-link href="{{ route('berita.index') }}" :active="request()->routeIs('berita.index')"
                class="text-gray-800 border-l-4 {{ request()->routeIs('berita.index') ? 'border-zinc-700!' : 'border-transparent' }}">
                {{ __('Berita') }}
            </x-responsive-nav-link>
        </div>
    </div>
